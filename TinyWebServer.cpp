...
size_t TinyWebServer::read(Client& client, uint8_t* buffer, size_t size) {
	if (!client.available()) {
		return false;
	} else {
...
size_t read_fast(TinyWebServer& web_server, Client& client, uint8_t* buffer, int size) {
	return web_server.read(client, buffer, size);
}
